...
- **M5 — 🔶 / ⏸.** ✅ Пины зависимостей + лог версий + запрет pickle-расширений (M5-2), ограничение роста lossHistory/logs (M5-4), сохраняемые пользовательские пресеты конфига (localStorage, M5-5 часть). ⏸ Осталось: тесты/lint/CI (нужен `npm install`), устойчивость (single-instance/логи/рестарт), auto-update, кроссплатформенность. Экран Settings — понижен: настройки (models/output dir, HF-токен) уже имеют UI в ModelsPage/GalleryPage, отдельный экран дублировал бы их. ⚠️ **M5-3 НЕ выполнен намеренно** — в `ConfigPage/DatasetPage/TrainingPage` лежат незакоммиченные правки; удаление затрёт их (решение за автором).
